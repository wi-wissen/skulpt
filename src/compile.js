...
Compiler.prototype.buildcodeobj = function (n, coname, decorator_list, args, callback, class_for_super) {
    ...
    this.u.varDeclsCode += ", $prevframe = Sk._frame, $args=undefined, $argNames=[];"; // declare $args to access global and $argNames to store names
    if (Sk.execLimit !== null) {
    ...
    if (fastCall) {
        ...
        if (!kwarg && !vararg && (!args || !args.kwonlyargs || args.kwonlyargs.length === 0)) {
            this.u.varDeclsCode += "$args = ((!$kwargs || $kwargs.length===0) && $posargs.length===" + funcArgs.length + ") ? $posargs : this.$resolveArgs($posargs,$kwargs)";
        } else {
            this.u.varDeclsCode += "\$args = this.$resolveArgs($posargs,$kwargs)\n";
        }
        for (let i = 0; i < funcArgs.length; i++) {
            this.u.varDeclsCode += "," + funcArgs[i] + "=$args[" + i + "]";
            this.u.varDeclsCode += ",$argNames.push('" + funcArgs[i] + "')"; // push arg names to array
        }
    ...
    this.u.varDeclsCode += "if ($argNames.length) Sk._frame.args = $argNames.reduce((acc, cur, i) => ({ ...acc, [cur]: $args[i] }), {});";
